        ...
        <div class="col-xs-12">
          <table id="myTable" class="table table-active">
            <tr class="header">
              ...
              <?php
                $user = $_SESSION["email"];
                $result = $mysqli->query("SELECT `booking`.bookingid, `users`.`full_name`, `booking`.`date`, `booking`.`time`, `booking`.`status`, `booking`.`email` FROM `booking` INNER JOIN  `users` ON `booking`.email = `users`.email WHERE `status` = 'pending' OR `status` = 'Please Reshedule' OR `status` = 'Resheduled' ORDER BY `booking`.bookingid");
                if($result) {
