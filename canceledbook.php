      ...
      <div class="row">
        <div class="col-xs-12">
          <table id="myTable" class="table table-active">
            ...
            <?php
            $user = $_SESSION["email"];
            $result = $mysqli->query("SELECT `booking`.bookingid, `users`.`full_name`, `booking`.`date`, `booking`.`time`, `booking`.`status` FROM `booking` INNER JOIN  `users` ON `booking`.email = `users`.email WHERE `status` = 'Canceled' ORDER BY `booking`.bookingid");
            if($result) {
